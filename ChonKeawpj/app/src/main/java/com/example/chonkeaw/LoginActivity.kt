...
class LoginActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        login_loginBtn.setOnClickListener {
            ...
            // ทำการตรวจสอบก่อนว่ามีข้อมูลหรือไม่ ก่อนที่จะไปตรวจสอบค่า
            if (email.isEmpty()) {
            ...
            //ทำการตรวจสอบค่าที่กรอกกับค่าจาก Firebase Authentication
            mAuth!!.signInWithEmailAndPassword(email, password).addOnCompleteListener { task ->  if (!task.isSuccessful) { //กรณีที่ไม่ผ่านการตรวจสอบ
                if (password.length < 6) { //ตรวจสอบความยาวของ password ว่าน้อยกว่า 6 ไหม  login_passwordEditText.error = "Please check your password. Password must have  minimum 6 characters."
